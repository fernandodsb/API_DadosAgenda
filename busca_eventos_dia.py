# busca_eventos_dia.py
# Script para buscar eventos do Google Calendar APENAS PARA O DIA ATUAL,
# processá-los e realizar UPSERT no banco de dados.

import datetime
import pytz
import sys
import re
import json  # Importado para o debug print, pode ser removido se não usado
import psycopg2

# Importa seus módulos de autenticação e banco de dados
try:
    import auth
    import database
except ImportError as e:
    print(
        f"Erro ao importar módulo: {e}. Certifique-se que auth.py e database.py estão no mesmo diretório ou no PYTHONPATH.")
    sys.exit(1)

from googleapiclient.errors import HttpError
from psycopg2 import OperationalError, Error

# --- Configurações (idênticas ao busca_eventos_semana.py) ---
MAIN_CALENDAR_ID = 'centraldeatendimento@crfgo.org.br'
SECTIONAL_CALENDAR_IDS = [
    'anapolis@crfgo.org.br',
    'luziania@crfgo.org.br',
    'rioverde@crfgo.org.br',
    'uruacu@crfgo.org.br',
]
CALENDAR_IDS = [MAIN_CALENDAR_ID] + SECTIONAL_CALENDAR_IDS
LOCAL_TIMEZONE_STR = 'America/Sao_Paulo'
LOCAL_TIMEZONE = pytz.timezone(LOCAL_TIMEZONE_STR)
UTC = pytz.utc


# --- Função para buscar eventos de um único calendário (reutilizada) ---
def fetch_events_from_calendar(service, calendar_id, time_min_rfc3339, time_max_rfc3339):
    """Busca eventos de um calendário específico em um range de tempo com paginação."""
    print(f"\n--- Buscando eventos no calendário: {calendar_id} (de {time_min_rfc3339} a {time_max_rfc3339}) ---")
    all_events = []
    page_token = None
    page_num = 1
    while True:
        try:
            events_result = service.events().list(
                calendarId=calendar_id,
                timeMin=time_min_rfc3339,
                timeMax=time_max_rfc3339,
                singleEvents=True,
                orderBy='startTime',
                pageToken=page_token,
                maxResults=100  # Busca em lotes de 100
            ).execute()
            events = events_result.get('items', [])
            if not events:
                print(f"  Nenhum evento encontrado na página {page_num} para {calendar_id}.")
                break
            print(f"  Encontrados {len(events)} eventos na página {page_num} para {calendar_id}.")
            all_events.extend(events)
            page_token = events_result.get('nextPageToken')
            if not page_token:
                print(f"  Paginação completa para {calendar_id}.")
                break
            page_num += 1
        except HttpError as error:
            print(f'  Ocorreu um erro na API ao buscar eventos do calendário {calendar_id}: {error}')
            break
        except Exception as e:
            print(f'  Ocorreu um erro inesperado ao buscar eventos do calendário {calendar_id}: {e}')
            break
    print(f"--- Busca concluída para o calendário {calendar_id}. Total: {len(all_events)} eventos. ---")
    return all_events


# --- Função para processar eventos e preparar para o BD (reutilizada e idêntica) ---
def process_events_for_db(events, calendar_id):
    """Processa a lista de eventos buscados, aplicando lógica condicional por calendário,
       e retorna uma lista de dicionários formatados internamente, com status_evento."""
    print(f"\n--- Processando {len(events)} eventos para o calendário: {calendar_id} ---")
    processed_data = []
    for event in events:
        event_summary = event.get('summary', '')
        google_event_status = event.get('status')
        db_status_evento = 'A'  # Default para Ativo

        if event_summary and isinstance(event_summary, str):
            normalized_summary = event_summary.strip().lower()
            if normalized_summary.startswith("cancelar:") or normalized_summary.startswith("cancelados:"):
                db_status_evento = 'C'
            elif google_event_status == 'cancelled':
                db_status_evento = 'C'

        event_data = {
            'id_google_calendar': event.get('id'),
            'calendar_id': calendar_id,
            'summary': event_summary,
            'description': event.get('description'),
            'location': event.get('location'),
            'html_link': event.get('htmlLink'),
            'status': google_event_status,
            'status_evento': db_status_evento,
            'created': event.get('created'),
            'updated': event.get('updated'),
            'servico_agendado': None,
            'attendees_emails_string': "",
            'is_full_day': False,
            'start_time': None,
            'end_time': None,
            'telefone': None,
            'numero_crf': None,
            'nome_convidado': None,
            'local_evento': None,
        }

        # Tratamento de Data/Hora (created, updated, start_time, end_time)
        try:
            created_str = event_data.get('created')
            if created_str:
                created_dt = datetime.datetime.fromisoformat(created_str.replace('Z', '+00:00'))
                event_data['created'] = created_dt.astimezone(UTC)
        except (ValueError, TypeError):
            event_data['created'] = None
        try:
            updated_str = event_data.get('updated')
            if updated_str:
                updated_dt = datetime.datetime.fromisoformat(updated_str.replace('Z', '+00:00'))
                event_data['updated'] = updated_dt.astimezone(UTC)
        except (ValueError, TypeError):
            event_data['updated'] = None

        start_info = event.get('start', {})
        end_info = event.get('end', {})
        if 'dateTime' in start_info:
            try:
                start_dt_str = start_info.get('dateTime')
                end_dt_str = end_info.get('dateTime')
                if start_dt_str:
                    start_dt = datetime.datetime.fromisoformat(start_dt_str)
                    event_data['start_time'] = start_dt.astimezone(UTC) if start_dt.tzinfo else LOCAL_TIMEZONE.localize(
                        start_dt).astimezone(UTC)
                if end_dt_str:
                    end_dt = datetime.datetime.fromisoformat(end_dt_str)
                    event_data['end_time'] = end_dt.astimezone(UTC) if end_dt.tzinfo else LOCAL_TIMEZONE.localize(
                        end_dt).astimezone(UTC)
            except (ValueError, TypeError):
                event_data['start_time'], event_data['end_time'] = None, None
        elif 'date' in start_info:  # Eventos de dia inteiro
            event_data['is_full_day'] = True
            try:
                start_date_obj = datetime.datetime.strptime(start_info.get('date'), '%Y-%m-%d')
                event_data['start_time'] = LOCAL_TIMEZONE.localize(start_date_obj).astimezone(UTC)
                # Para eventos de dia inteiro, 'end' é exclusivo.
                # Se o evento dura 1 dia, end_date será o dia seguinte ao start_date.
                end_date_str = end_info.get('date')
                if end_date_str:
                    end_date_obj = datetime.datetime.strptime(end_date_str, '%Y-%m-%d')
                    event_data['end_time'] = LOCAL_TIMEZONE.localize(end_date_obj).astimezone(UTC)
                else:  # Fallback se 'end.date' não estiver presente (improvável para singleEvents=True)
                    event_data['end_time'] = (
                                LOCAL_TIMEZONE.localize(start_date_obj) + datetime.timedelta(days=1)).astimezone(UTC)
            except (ValueError, TypeError):
                event_data['start_time'], event_data['end_time'] = None, None

        # Lógica de extração de dados da descrição e summary (idêntica)
        description = event_data.get('description')
        if calendar_id == MAIN_CALENDAR_ID:
            if description and isinstance(description, str):
                servico_match = re.search(r"Selecione o serviço a ser agendado::\s*(.*?)\n", description)
                email_match = re.search(r"E-mail do convidado:\s*(.*?)\n", description)
                nome_match = re.search(r"Convidado:\s*(.*?)\n", description)
                telefone_match = re.search(r"Telefone:\s*(.*?)\n", description)
                crf_match = re.search(r"Inscrição \(Nº CRF\):\s*(.*?)\n", description)
                if servico_match: event_data['servico_agendado'] = servico_match.group(1).strip()
                if email_match: event_data['attendees_emails_string'] = email_match.group(1).strip()
                if nome_match: event_data['nome_convidado'] = nome_match.group(1).strip()
                if telefone_match: event_data['telefone'] = telefone_match.group(1).strip()
                if crf_match: event_data['numero_crf'] = crf_match.group(1).strip()
        elif calendar_id in SECTIONAL_CALENDAR_IDS:
            if description and isinstance(description, str):
                servico_match = re.search(r"Selecione o serviço a ser agendado::\s*(.*?)\n", description, re.IGNORECASE)
                if servico_match: event_data['servico_agendado'] = servico_match.group(1).strip()
                telefone_match_desc = re.search(r"Telefone:\s*(.*?)\n", description)
                if telefone_match_desc: event_data['telefone'] = telefone_match_desc.group(1).strip()
                crf_match_desc = re.search(r"Inscrição \(Nº CRF\):\s*(.*?)\n", description)
                if crf_match_desc: event_data['numero_crf'] = crf_match_desc.group(1).strip()

            summary_for_name = event_data.get('summary', '')
            if summary_for_name and isinstance(summary_for_name, str) and summary_for_name.strip().lower().startswith(
                    "cancelar:"):
                summary_for_name = summary_for_name.replace("Cancelar:", "", 1).strip()
            if summary_for_name:
                nome_match_summary = re.match(r"(.*?)\s+e\s+CRF-GO", summary_for_name, re.IGNORECASE)
                if nome_match_summary:
                    event_data['nome_convidado'] = nome_match_summary.group(1).strip()
                else:
                    parts = summary_for_name.split(' e ', 1)
                    event_data['nome_convidado'] = parts[0].strip() if parts and parts[
                        0].strip() else summary_for_name.strip()

            attendees = event.get('attendees')
            if attendees:
                calendar_emails = [MAIN_CALENDAR_ID.lower()] + [cid.lower() for cid in SECTIONAL_CALENDAR_IDS]
                for att in attendees:
                    email = att.get('email')
                    if email and email.lower() not in calendar_emails:
                        event_data['attendees_emails_string'] = email
                        break

        # Pós-processamento comum (limpeza, validação, local_evento, truncamento)
        if event_data.get('telefone') is not None and isinstance(event_data['telefone'], str):
            event_data['telefone'] = re.sub(r'[^\d\+]', '', event_data['telefone'])

        numero_crf = event_data.get('numero_crf')
        if numero_crf is not None and isinstance(numero_crf, str):
            cleaned_crf = numero_crf.strip()
            if cleaned_crf in ["0", "00", "000", "0000", "00000"] or len(cleaned_crf) > 5:
                event_data['numero_crf'] = None
            else:
                event_data['numero_crf'] = cleaned_crf

        location_map = {
            MAIN_CALENDAR_ID: 'Goiânia', 'anapolis@crfgo.org.br': 'Anápolis',
            'luziania@crfgo.org.br': 'Luziânia', 'rioverde@crfgo.org.br': 'Rio Verde',
            'uruacu@crfgo.org.br': 'Uruaçu',
        }
        event_data['local_evento'] = location_map.get(event_data.get('calendar_id'))

        string_fields_to_default = ['summary', 'description', 'location', 'html_link', 'status', 'servico_agendado',
                                    'attendees_emails_string', 'nome_convidado', 'telefone', 'numero_crf']
        for field in string_fields_to_default:
            if event_data.get(field) is None or (isinstance(event_data[field], str) and not event_data[field].strip()):
                if field not in ['numero_crf', 'telefone'] or event_data.get(field) is not None:
                    event_data[field] = "Não Informado" if field == 'servico_agendado' else ""
        if not event_data.get('status_evento'): event_data['status_evento'] = 'A'

        field_limits_db = {  # Limites de caracteres para colunas do BD
            'id_google_calendar': 255, 'servico_agendado': 255, 'attendees_emails_string': 255,
            'telefone': 50, 'numero_crf': 50, 'nome_convidado': 255, 'summary': 500,
            'location': 500, 'html_link': 500, 'status': 50, 'status_evento': 1,
            'description': None,  # TEXT no BD, sem limite prático aqui
            'calendar_id': 255, 'local_evento': 100
        }
        for field, max_len in field_limits_db.items():
            if max_len is not None and event_data.get(field) and isinstance(event_data[field], str) and len(
                    event_data[field]) > max_len:
                # print(f"  AVISO: Campo '{field}' truncado para {max_len} caracteres.")
                event_data[field] = event_data[field][:max_len]

        processed_data.append(event_data)
    return processed_data


# --- Lógica principal ---
if __name__ == '__main__':
    print(f"--- {datetime.datetime.now()} - Iniciando Busca de Eventos do DIA ATUAL ---")

    try:
        # --- 1. Calcular range de datas (APENAS DIA ATUAL) ---
        print("\n--- Calculando range de datas (Dia Atual) ---")
        today_date = datetime.date.today()

        # Início do dia atual (00:00:00) no fuso horário local
        start_of_day_local = datetime.datetime.combine(today_date, datetime.time.min)
        # Fim do dia atual (23:59:59.999999) no fuso horário local
        end_of_day_local = datetime.datetime.combine(today_date, datetime.time.max)

        # Converte para fuso horário local com informação de timezone
        local_aware_start = LOCAL_TIMEZONE.localize(start_of_day_local)
        local_aware_end = LOCAL_TIMEZONE.localize(end_of_day_local)

        # Converte para UTC para a API do Google
        utc_aware_start = local_aware_start.astimezone(UTC)
        utc_aware_end = local_aware_end.astimezone(UTC)

        # Formato RFC3339 para a API
        time_min_rfc3339 = utc_aware_start.isoformat()
        time_max_rfc3339 = utc_aware_end.isoformat()

        print(f"  Timezone Local: {LOCAL_TIMEZONE_STR}")
        print(f"  Dia Atual Local: {today_date.strftime('%Y-%m-%d')}")
        print(f"  Range Local: De {local_aware_start} até {local_aware_end}")
        print(f"  Range para API (UTC/RFC3339): De {time_min_rfc3339} até {time_max_rfc3339}")

        # --- 2. Autenticar com Google Calendar API ---
        print("\n--- Autenticando com Google Calendar API ---")
        service = auth.get_calendar_service()
        if not service:
            print("Falha ao obter o serviço do Google Calendar. Saindo.")
            sys.exit(1)

        # --- 3. Buscar e processar eventos ---
        all_processed_events_for_db = []
        print(f"\n--- Buscando e processando eventos de {len(CALENDAR_IDS)} calendários (Dia Atual)... ---")
        for calendar_id in CALENDAR_IDS:
            events = fetch_events_from_calendar(service, calendar_id, time_min_rfc3339, time_max_rfc3339)
            if events:
                processed_events = process_events_for_db(events, calendar_id)
                all_processed_events_for_db.extend(processed_events)
        print(
            f"\n--- Busca e processamento concluído. Total de eventos do dia para BD: {len(all_processed_events_for_db)} ---")

        # --- 4. Conectar ao Banco de Dados e realizar UPSERT ---
        print("\n--- Conectando ao Banco de Dados ---")
        db_conn, db_cursor = None, None
        try:
            db_conn, db_cursor = database.connect_db()
            if not db_conn or not db_cursor:
                print("Falha ao conectar ao BD. Os dados NÃO serão salvos.")
            else:
                print("Conexão com o BD estabelecida.")
                print(f"\nRealizando UPSERT de {len(all_processed_events_for_db)} eventos no BD...")
                if not all_processed_events_for_db:
                    print("Nenhum evento processado para UPSERT.")
                else:
                    upsert_sql = """
                   INSERT INTO calendar_events (
                       google_event_id, servico_agendado, email_farmaceutico, data_evento,
                       telefone_farmaceutico, inscricao_farmaceutico, nome_farmaceutico,
                       local_evento, fetched_at, status_evento
                    )
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (google_event_id)
                    DO UPDATE SET
                       servico_agendado = EXCLUDED.servico_agendado,
                       email_farmaceutico = EXCLUDED.email_farmaceutico,
                       data_evento = EXCLUDED.data_evento,
                       telefone_farmaceutico = EXCLUDED.telefone_farmaceutico,
                       inscricao_farmaceutico = EXCLUDED.inscricao_farmaceutico,
                       nome_farmaceutico = EXCLUDED.nome_farmaceutico,
                       local_evento = EXCLUDED.local_evento,
                       fetched_at = EXCLUDED.fetched_at,
                       status_evento = EXCLUDED.status_evento;
                   """
                    current_utc_time = datetime.datetime.now(UTC)
                    data_to_upsert = [
                        (
                            event_data.get('id_google_calendar'),
                            event_data.get('servico_agendado'),
                            event_data.get('attendees_emails_string'),
                            event_data.get('start_time'),  # data_evento
                            event_data.get('telefone'),
                            event_data.get('numero_crf'),
                            event_data.get('nome_convidado'),
                            event_data.get('local_evento'),
                            current_utc_time,  # fetched_at
                            event_data.get('status_evento')
                        )
                        for event_data in all_processed_events_for_db
                    ]
                    if data_to_upsert:
                        db_cursor.executemany(upsert_sql, data_to_upsert)
                        db_conn.commit()
                        print(f"UPSERT de {len(data_to_upsert)} eventos concluído com sucesso.")
                    else:
                        print("Nenhum dado válido para UPSERT.")
        except (OperationalError, Error, Exception) as e_db:
            if db_conn: db_conn.rollback()
            print(f"\n--- ERRO durante operação com BD: {e_db} ---")
            import traceback

            traceback.print_exc()
        finally:
            if db_cursor: db_cursor.close()
            if db_conn: db_conn.close()
            print("Conexão com BD fechada (se aberta).")

    except Exception as e_main:
        print(f"\n--- ERRO INESPERADO NO PROCESSO PRINCIPAL: {e_main} ---")
        import traceback

        traceback.print_exc()

    print(f"\n--- {datetime.datetime.now()} - Processo de Busca de Eventos do Dia Concluído ---")
